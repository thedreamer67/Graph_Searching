...
//		}
	...
	public static void printpath(int vertex) {
		...
		if (vertex == -1) {
			...
			System.out.println("Such a node id does not exist OR There is no path to a hospital from this node");
		}
		else {
			if (parent[vertex] == -1) {
	...
	public static void printpath_feed(int vertex, PrintWriter pw) {
			...
			if (vertex == -1) {
				...
				pw.println("Such a node id does not exist OR There is no path to a hospital from this node");
			}
			else {
				if (parent[vertex] == -1) {
					pw.print(vertex);
